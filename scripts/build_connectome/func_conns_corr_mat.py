"""
This script calculates correlations between three functional conn estimates from movie and resting state of each subject and then plots the diagonals as bar plots for each variation.
"""
...
movie_sub_ses_fmri = dict([(subject_sessions_fmri[2 * i][0],
[subject_sessions_fmri[2 * i][1], subject_sessions_fmri[2 * i + 1][1]])
for i in range(len(subject_sessions_fmri) // 2)])

resting_sub_ses_fmri = {'sub-01': ['ses-14','ses-15'], 'sub-04': ['ses-11','ses-12'], 'sub-05': ['ses-11','ses-12'], 'sub-06': ['ses-11','ses-12'], 'sub-07': ['ses-11','ses-12'], 'sub-08': ['ses-12','ses-13'], 'sub-09': ['ses-12','ses-13'], 'sub-11': ['ses-11','ses-12'], 'sub-12': ['ses-11','ses-12'], 'sub-13': ['ses-11','ses-12'], 'sub-14': ['ses-11','ses-12'], 'sub-15': ['ses-14','ses-15']}
    ...
    for sub_movie, ses_movies in movie_sub_ses_fmri.items():
        ...
        if sess_sift[0] == 'pearson':
            ...
            for ses_movie in ses_movies:
                runs = glob(os.path.join(DATA_ROOT, sub_movie, ses_movie, 'func', 'connectivity_tmp', 'schaefer400_corr*ses*.csv'))
                for run in runs:
                    movie_func = pd.read_csv(run, names=atlas.labels)
                    if hemisphere == 'LH':
                        movie_func = movie_func[0:200]
                    elif hemisphere == 'RH':
                        movie_func = movie_func[200:]
                    movie_func = connectome.sym_matrix_to_vec(movie_func.to_numpy(), discard_diagonal=True)
            ...
            func_mat = os.path.join(DATA_ROOT, sub_movie, 
                                    ses_movies[1],
...
diags_df = pd.DataFrame(data=diags, columns=[f'{similarity_metric} corr movie vs. resting-state func conn'])
...
off_diags_df = pd.DataFrame(data=off_diags, columns=[f'{similarity_metric} corr movie vs. resting-state func conn'])
...
fig = plt.figure(figsize=(10,10))
title = f''
sns.boxplot(df, orient = 'h', x=f'{similarity_metric} corr movie vs. resting-state func conn', y='func conn estimates', hue='comparison', fliersize=0)
plt.title(title)
