...
if __name__ == "__main__":
    ...
    for sub, ses in sub_ses.items():
        ...
        mni_t2 = os.path.join(
            DATA_ROOT, sub, ses, "dwi", "tract2mni_tmp", "mni_t2w.nii.gz"
        )
        os.system(f"cp {mni_t2} {tmp_dir}")
        mni_nifti = os.path.join(
            DATA_ROOT, sub, ses, "dwi", "atlas2dwi", "mni_t2w.nii.gz"
        )
        ...
        dwi_tck = os.path.join(
            DATA_ROOT, sub, ses, "dwi", f"tracks_{sub}_{ses}_t1.tck"
        )
        connectivity_matrix = os.path.join(
            tmp_dir, f"{atlas.name}_connectome_{sub}_{ses}_t2.csv"
        )
        inverse_connectivity_matrix = os.path.join(
            tmp_dir,
            f"{atlas.name}_inverse-connectome_{sub}_{ses}_t2.csv",
        )
        tck2connectome(
            atlas_in_dwi_space,
            dwi_tck,
        ...
        connectivity_matrix_sift = os.path.join(
            tmp_dir,
            (
        ...
        inverse_connectivity_matrix_sift = os.path.join(
            tmp_dir,
            (
        ...
        sift_weights = os.path.join(
            DATA_ROOT, sub, ses, "dwi", f"sift-track_{sub}_{ses}.txt"
        )
        tck2connectome(
            atlas_in_dwi_space,
            dwi_tck,
            connectivity_matrix_sift,
            inverse_connectivity_matrix_sift,
            sift_weights,
        )
        ...
        connectome_fig = os.path.join(
            tmp_dir, f"{atlas.name}_connectome_{sub}_{ses}_t2.png"
        )
        ...
        plotting.plot_matrix(
            mat_sift, labels=atlas.labels, figure=mat_sift_fig
        )
        connectome_sift_fig = os.path.join(
            tmp_dir,
            f"{atlas.name}_connectome_sift-weighted_{sub}_{ses}_t2.png",
        )
