...
if __name__ == "__main__":
    ...
    for sub, ses in sub_ses.items():
        ...
        if not os.path.exists(tmp_dir):
            os.makedirs(tmp_dir)
        schaefer400 = datasets.fetch_atlas_schaefer_2018(data_dir=tmp_dir, 
                                                         resolution_mm=1,
                                                         n_rois=400)
        ...
        connectivity_matrix = os.path.join(DATA_ROOT, sub, ses, 'dwi',
                                           f'schaefer400_connectome_{sub}_{ses}'
                                           '.csv')
        ...
        tck2connectome(schaefer400.maps, mni_tck, connectivity_matrix_sift,
                       inverse_connectivity_matrix_sift, sift_weights)
        mat = pd.read_csv(connectivity_matrix, names=schaefer400.labels)
        plt.figure(figsize = (50,50))
        mat_fig = sns.heatmap(mat, xticklabels=1).get_figure()
        mat_fig.savefig(os.path.join(tmp_dir,
                                     f'schaefer400_connectome_{sub}_{ses}.pdf'),
                                     bbox_inches='tight')
        mat_sift = pd.read_csv(connectivity_matrix_sift,
                               names=schaefer400.labels)
        plt.figure(figsize = (50,50))
        mat_sift_fig = sns.heatmap(mat_sift, xticklabels=1).get_figure()
        mat_sift_fig.savefig(os.path.join(tmp_dir,
                                          (f'schaefer400_connectome_sift-'
                                           f'weighted_{sub}_{ses}.pdf')),
                                          bbox_inches='tight')
