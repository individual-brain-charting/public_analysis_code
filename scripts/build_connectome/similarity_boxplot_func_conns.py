    ...
    for similarity_metric in similarity_metrics:
        for struc_conn in struc_connectivity_matrices:
            ...
            for func_type in func_types:
                ...
                for func_conn in func_connectivity_matrices:
                    ...
                    for sub_dwi, ses_dwi in sub_ses_dwi.items():
                        ...
                        if space_sift[0] == "native":
                            if space_sift[1] == "sift":
                                filename = (
                                    f"sift-weighted_{sub_dwi}_{ses_dwi}_t2.csv"
                                )
                            elif space_sift[1] == "no":
                                filename = f"{sub_dwi}_{ses_dwi}_t2.csv"
                            struc_mat = os.path.join(
                                DATA_ROOT,
                                sub_dwi,
                                ses_dwi,
                                "dwi",
                        ...
                        if hemisphere == "LH":
                            ...
                            struc = struc[200:]
                        struc = connectome.sym_matrix_to_vec(
                            struc.to_numpy(), discard_diagonal=True
                        )
                        ...
                        for sub_func, sess_funcs in sub_ses_fmri.items():
                            ...
                            if sess_sift[0] == "pearson":
                                ...
                                if sess_sift[1] == "cov":
                                    filename = (
                                        f"schaefer400_corr_{sub_func}_all.csv"
                                    )
                            ...
                            if similarity_metric == "cosine":
                                similarity = pairwise.cosine_similarity(
                                    struc, func
                                )
                                similarity = similarity[0][0]
                            elif similarity_metric == "pearson":
                                similarity = np.corrcoef(struc, func)
                                similarity = np.hsplit(
                                    np.vsplit(similarity, 2)[0], 2
                                )[1]
                                similarity = similarity[0][0]
                            elif similarity_metric == "spearman":
                                similarity, _ = stats.spearmanr(
                                    struc, func, axis=1
                                )
                    ...
                    matrix_df = pd.DataFrame(
                        all_sub_similarities, columns=subs, index=subs
                    )
                    ...
                    plt.savefig(
                        f"similarity_mat_{func_conn}.png", bbox_inches="tight"
                    )
                    plt.close("all")

                    all_sub_similarities_norm = (
                        all_sub_similarities
                        - all_sub_similarities.mean(axis=0)
                    )
                    all_sub_similarities_norm = (
                        all_sub_similarities_norm.T
                        - all_sub_similarities_norm.mean(axis=1)
                    ).T
                    matrix_df = pd.DataFrame(
                        all_sub_similarities_norm, columns=subs, index=subs
                    )
                    fig = plt.figure(figsize=(10, 10))
                    sns.heatmap(matrix_df)
                    ...
                    plt.ylabel("struc conn")
                    plt.savefig(
                        f"similarity_mat_{func_conn}_mean_centered.png",
                        bbox_inches="tight",
                    )
                    plt.close("all")

                    off_diag = all_sub_similarities[
                        np.where(
                            ~np.eye(all_sub_similarities.shape[0], dtype=bool)
                        )
                    ]
                    off_diags.append(off_diag)
                    off_diag_labels.append(
                        [
                            f"{func_type}_{func_conn}"
                    ...
                    diags.append(diag)
                    diag_labels.append(
                        [f"{func_type}_{func_conn}" for i in range(len(diag))]
                    )

                    off_diag_norm = all_sub_similarities_norm[
                        np.where(
                            ~np.eye(
                                all_sub_similarities_norm.shape[0], dtype=bool
                            )
                        )
                    ]
                    off_diags_norm.append(off_diag_norm)
                    off_diag_labels_norm.append(
                        [
                            f"{func_type}_{func_conn}"
                            for i in range(len(off_diag_norm))
                        ]
                    )
                    diag_norm = np.diagonal(all_sub_similarities_norm)
                    diags_norm.append(diag_norm)
                    diag_labels_norm.append(
                        [
                            f"{func_type}_{func_conn}"
                            for i in range(len(diag_norm))
                        ]
                    )

                    print(f"{func_type}_{func_conn}")
                    t_test = stats.ttest_ind(
                        diag_norm, off_diag_norm, alternative="greater"
                    )
            ...
            diags_df = pd.DataFrame(
                data=diags,
                columns=[f"{similarity_metric} corr func conn vs. struc conn"],
            )
            diags_df["func conn estimates"] = diag_labels
            diags_df["comparison"] = [
                "same subject" for i in range(len(diags_df))
            ]
            ...
            sns.boxplot(
                df,
                orient="h",
                x=f"{similarity_metric} corr func conn vs. struc conn",
            ...
            plt.title(title)
            plt.savefig(
                "similarity_boxplot_func_conns_gsc.png", bbox_inches="tight"
            )
            plt.close()
            ...
            diags_df = pd.DataFrame(
                data=diags_norm,
                columns=[
                    f"double mean centered {similarity_metric} corr func conn vs. struc conn"
                ],
            )
            ...
            sns.boxplot(
                df,
                orient="h",
                x=f"double mean centered {similarity_metric} corr func conn vs. struc conn",
                y="func conn estimates",
                hue="comparison",
                fliersize=0,
            )
            plt.title(title)
            plt.savefig(
                "similarity_boxplot_func_conns_gsc_mean_center.png",
                bbox_inches="tight",
            )
            plt.close()
