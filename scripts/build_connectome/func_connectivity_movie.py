...
functional connectivity) from movie (Raiders) fMRI for ROIs from a given atlas
"""
...
subject_sessions = sorted(get_subject_session(['raiders1', 'raiders2']))
...
for sub, sess in sub_ses.items():
    ...
    for ses in sess:
        ...
        for run in runs:
            ...
            confounds =  glob(os.path.join(DATA_ROOT, sub, ses, 'func', f'rp*{run_num}*'))[0]
            ...
            glc.fit(time_series)
            # save partial correlation matrix as csv
