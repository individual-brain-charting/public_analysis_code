...
for the visual Short-Term Memory task
...
from behav_utils import generate_csv


def vstm_scores_extractor(participants, dir_path, numerosity = None):
    all_pt_scores = []
    all_pt_means = []
    ...
    for participant in participants:
        ...
        for log_fname in log_files:
            ...
            discarded_trials = len(log_file[1:]) % 72
            ...
            for dt, data in enumerate(data_list):
                if numerosity is not None:
                    if numerosity == int(data[6]):
                        correct_answers.append(data[12])
                else:
                    correct_answers.append(data[12])
                # Compute scores for each run and append to all_scores
                if dt in [71, 143]:
                    correct_answers = [x.strip(' ') for x in correct_answers]
                    new_correct_answers = np.array(['0' if y=="99" else y
                                           for y in correct_answers])
                    score_list = [float(i) for i in new_correct_answers]
                    score = np.sum(score_list)
                    total_score = (score / new_correct_answers.size) * 100
                    all_scores.append(np.rint(total_score))
...
participants_scores, runs_id, gmean_all, gstd_all = vstm_scores_extractor(
    pt_list, parent_dir)
...
num = [1, 2, 3, 4, 5, 6]
for n in num:
    print('Numerosity = ' + str(n))
    participants_scores, runs_id, gmean, gstd = vstm_scores_extractor(
        pt_list, parent_dir, numerosity = n)
